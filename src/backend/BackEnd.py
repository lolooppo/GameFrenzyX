...
class BackEnd:
    """Facade pattern: Coordinates all backend operations — manages users, reservables, and database persistence."""

    def __init__(self, users_file_path, reservables_file_path):
        # Store file paths for data persistence
    ...
    def __del__(self):
        # Save data when backend is destroyed
    ...
    def reserve(self, user_name: str, reservation_request: ReservationRequest) -> bool:
        ...
        if not is_reserved:
            return False
        # If successful, add to user's reservations
    ...
    def cancel_user_reservation(self, user_name, reservation_request: ReservationRequest) :
        # Remove from user's reservations and unreserve the item
